...
const EditableCell = ({ value: initialValue, onSave, isEditing, onToggleEditing, type = 'text', options = [] }: EditableCellProps) => {
    ...
    if (isEditing) {
        ...
        switch (type) {
            ...
            case 'number': return ( <Input type="number" value={value as number ?? ''} onChange={(e) => setValue(parseFloat(e.target.value) || 0)} onBlur={handleSave} onKeyDown={handleKeyDown} autoFocus className="h-8 text-xs md:text-sm"/> ); // Added ?? '' for controlled input warning
            case 'textarea':
                return (
                    <Textarea
                        ref={textareaRef}
    ...
    else if (type === 'number') { displayValue = formatCurrency(value as number | null); } // Use formatCurrency here
    ...
    else if (value === null || value === undefined || value === '') { displayValue = '-'; } // Display '-' for empty text fields too
    ...
    const formatDateField = (field: keyof Servico | keyof Despesa) => {
        if (field in dataToInsert) {
            ...
            if (dateValue instanceof Date) {
                ...
                try { (dataToInsert as any)[field] = format(parseDateStringAsLocal(dateValue)!, 'yyyy-MM-dd'); }
                catch { (dataToInsert as any)[field] = null; }
